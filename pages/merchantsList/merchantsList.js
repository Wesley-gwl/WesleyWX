...
Page({
  ...
  addCustomer:function(){
    this.setData({ show: false });
    wx.navigateTo({
      url: '/pages/merchantsAddOrEdit/merchantsAddOrEdit',
    })
  },
  //新增
  addCustomerType:function(){
    this.setData({ show: false });
    wx.navigateTo({
      url: '/pages/customerTypeAddOrEdit/customerTypeAddOrEdit?customer='+JSON.stringify({type:2,typeName:'来往单位'}),
    })
  },
  //查看编辑详情
  onLookCustomerInfo:function(event){
    ...
    if(customer!={}){
      wx.navigateTo({
        url: '/pages/merchantsAddOrEdit/merchantsAddOrEdit?customer=' + JSON.stringify(customer),
      })
    }
  },
  /**
