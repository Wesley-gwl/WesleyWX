...
Page({
  ...
  onSwichCheck:function(event){
    ...
    switch (position) {
      case 'left':
        that.deleteAccountHX(event);
        break;
      case 'cell':
        break;
      case 'right':
        that.onLookAccountHXInfo(event);
        break;
    }
    instance.close();
  },
  //删除
  deleteAccountHX:function(event){
  ...
  DeleteApply:function(id){
    wx.request({
      url: config.deleteAccountHX_url,
      method: 'post',
      dataType:"json",
      ...
      success(res) {
        if(res.data.success){
          ...
          that.getAccountHXList();
        }
      }
    })
  },
  //查看编辑详情
  onLookAccountHXInfo:function(event){
    var id = event.currentTarget.dataset.id;
    wx.navigateTo({
      url: '/pages/purchaseAccountHXAddOrEdit/purchaseAccountHXAddOrEdit?id=' + id,
    })
  },
  /**
   * 生命周期函数--监听页面加载
  ...
  getAccountHXList:function(){
    ...
    wx.request({
      url: config.getAccountHXList_url,
      method: 'post',
      dataType: "json",
      header: header,//传在请求的header里
